...
class CompanyController extends Controller {
  ...
  registerRep(req, res, next) {
    const { companyName, firstName, lastName, email, password } = req.body;
    const role = 'COMPANY_REP';

    const newUserAndCompanyQuery = [
      companyFacade.userSchema().create({ firstName, lastName, role, email, password }),
      companyFacade.find({ companyName })
    ];

    Promise.all(newUserAndCompanyQuery).then(exists => exists).then((exists) => {
      // link them.
    });
